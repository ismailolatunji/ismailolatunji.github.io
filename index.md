---
---

# Ismail Olatunji

**Data and BI Analyst | KPI Architecture | SQL | Python | Power BI**  
7+ Years Commercial Analytics Experience  
Manchester, UK • [LinkedIn](https://linkedin.com/in/ismailolatunji) • [GitHub](https://github.com/ismailolatunji)

---

## Profile

Data and BI Analyst with 7+ years of commercial experience designing KPI frameworks, reporting architectures and performance analytics solutions across marketing, operations and executive functions.

I specialise in translating business objectives into measurable performance metrics, building structured data pipelines and delivering dashboards that support strategic decision making.

My portfolio includes commercial analytics and public sector benchmarking projects using real UK datasets.

---

## Featured Projects

### Education Attainment Gap Analysis  
![EHCP Preview](assets/medway_vs_national_ehcp_trend.png)

Benchmark analysis of EHCP phonics attainment against national averages across six academic years.

• Identified sustained 11 percent attainment gap  
• Highlighted data quality inconsistencies  
• Delivered board ready reporting to support targeted intervention planning  

Tech: Excel, SQL, Python  
[View Repository](https://github.com/ismailolatunji/EHCP-phonics-attainment-gap-analysis-medway)

---

### Road Traffic Collision Severity Modelling  
![Collision Dashboard Preview](assets/Road_Traffic_Collision.png)

Machine learning modelling and interactive dashboard analysing 22,000+ UK collision records.

• Built and compared five ML models  
• Achieved 97 percent classification accuracy  
• Identified key severity risk factors and geographic hotspots  

Tech: Python, Scikit Learn, SQL, Power BI  
[View Repository](https://github.com/ismailolatunji/Road-Traffic-Collision-Analysis-in-Cambridgeshire-UK)

---

### Renewable Installations and Generation Benchmark  
![Renewable Dashboard Preview](assets/CPCA_Renewable_Dashboard.png)

Regional renewable energy benchmarking across ten years of installations and generation trends.

• Analysed 2014 to 2024 growth trajectory  
• Benchmarked regional contribution against UK totals  
• Delivered reusable monitoring framework  

Tech: Excel, SQL, Python  
[View Repository](https://github.com/ismailolatunji/cpca-renewable-energy-installations-generation-analysis)

---

### UK Crime Analysis Using PySpark

Large scale crime dataset analysis identifying geographic concentration patterns and long term trends.

• Processed multi year dataset using Apache Spark  
• Identified concentration hotspots  
• Delivered structured insights for policy level discussion  

Tech: PySpark, SQL, Python  
[View Repository](https://github.com/ismailolatunji/Crime-Analysis-Using-PySpark-and-Python)

---

### Executive Sales Performance Dashboard  
![Sales Dashboard Preview](assets/Sales_Performance_Analysis.png)

Automated Excel dashboard providing real time revenue and product performance visibility.

• Reduced reporting time by 40 percent  
• Enabled autonomous cross functional access  
• Improved executive level performance tracking  

Tech: Excel, Power Query, VBA  
[View Repository](https://github.com/ismailolatunji/Interactive-Excel-Dashboard-In-Depth-Sales-Performance-Analysis)

---

## In Development

### Subscription Revenue Intelligence Framework

Designing an end to end KPI framework for a subscription SaaS business including revenue modelling, churn analysis, metric definitions and SQL data modelling.

This project will demonstrate production ready KPI architecture applied to a realistic commercial context.

---

## Contact

Open to Data Analyst, BI Analyst and Insight Analyst roles across the UK.

Email: olatunjiomotayoh@yahoo.com  
LinkedIn: https://linkedin.com/in/ismailolatunji
